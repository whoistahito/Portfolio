...
- **Desktop (>768px)**: Artworks positioned absolutely on fixed-width canvas (1440px × variable height)
- **Mobile (≤768px)**: Conditional rendering **within the same component file** using inline `if (isMobile)` blocks
- All screens implement `useState(() => window.innerWidth <= 768)` with resize listeners
- **No separate mobile files** — mobile variants are inlined in the same screen/component (e.g., `HomePage.tsx` contains both desktop and mobile gallery logic)

Example from `src/screens/contact.tsx`:
```tsx
if (isMobile) {
...
- **SiteHeader** (desktop) / **SiteHeaderMobile** (mobile): Sticky navigation with dynamic top offset
  - Desktop: 41px language bar above nav, collapses on scroll
...
### Production Build & Deployment (Heroku)
```bash
npm run build        # Generates dist/ with Vite
node server.js       # Express serves dist/ with SPA fallback
```
...
- **No GitHub Pages** — migrated from Pages to Heroku (see `DEPLOYMENT.md`)
...
Artworks defined as typed arrays in screen files (e.g., `HomePage.tsx`):
```tsx
type Artwork = {
  id: number;
  ...
  img: { w: number; h: number; top: number; left: number }; // Desktop absolute coords
  caption?: { w?: number; top: number; left: number; align?: "right" | "left" };
};
```
- **Desktop**: Render with `style={{ position: 'absolute', width, height, top, left }}`
- **Mobile**: Ignore coords, use flow layout with responsive widths

### Mobile Dynamic Stacking
Mobile screens use ResizeObserver to calculate container height:
```tsx
...
```
This pattern appears in `contact.tsx`, `exhebitions.tsx`, `aboutMe.tsx`, and `HomePage.tsx` mobile blocks.
...
2. **Absolute positioning**: Desktop artwork coords are hand-tuned; changing canvas width (1440px) breaks layout
...
6. **Mobile stacking height**: Initial guess must be reasonable to avoid layout shift; ResizeObserver adjusts after render
...
3. Add entry to `artworks` array with absolute coords (measure manually)
4. Adjust canvas height (`HomePage` root div) to accommodate new bottom coord
...
**For more context**: See `summary.md` (project exploration), `DEPLOYMENT.md` (Heroku setup).
